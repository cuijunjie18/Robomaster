...
DetectorTRT::DetectorTRT(const std::string &config_file, const std::string &share_dir,
    ...
    : NetDetector(config_file, share_dir, _logger), trt_logger(_logger) {
...
void DetectorTRT::set_cuda_context(CUcontext *ctx) { cuda_ctx = ctx; }
...
std::vector<Armor> DetectorTRT::detect(cv::Mat img) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(buffers[0], input, INPUT_H * INPUT_W * 3 * sizeof(float),
    ...
    for (int i = 0; i < layer_num; ++i) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(outputs[i], buffers[i + 1], out_size[i] * sizeof(float),
    ...
    for (int i = 0; i < layer_num; ++i) {
        decoder->decode(i, outputs[i], res);
    }
