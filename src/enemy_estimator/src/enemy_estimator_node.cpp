...
EnemyEstimatorNode::EnemyEstimatorNode(const rclcpp::NodeOptions& options) : Node("enemy_estimator", options) {
    ...
    std::string shared_dir = ament_index_cpp::get_package_share_directory("enemy_estimator");
    ...
    detection_sub = this->create_subscription<rm_interfaces::msg::Detection>(
        params.detection_name, rclcpp::SensorDataQoS(), std::bind(&EnemyEstimatorNode::detection_callback, this, std::placeholders::_1));

    robot_sub = this->create_subscription<rm_interfaces::msg::Rmrobot>(params.robot_name, rclcpp::SensorDataQoS(),
                                                                       std::bind(&EnemyEstimatorNode::robot_callback, this, std::placeholders::_1));
    states_pub = this->create_publisher<rm_interfaces::msg::EnemyStates>(params.robot_name + "_states", rclcpp::SensorDataQoS());
}

EnemyEstimatorNode::~EnemyEstimatorNode() {}

void EnemyEstimatorNode::load_params() {
...
RCLCPP_COMPONENTS_REGISTER_NODE(enemy_estimator::EnemyEstimatorNode)